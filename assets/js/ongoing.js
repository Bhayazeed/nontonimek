...
async function loadOngoingAnime(page) {
  const container = document.getElementById("ongoing-list-container");
  const paginationContainer = document.getElementById("ongoing-pagination");
  ...
  try {
    ...
    if (animes.length > 0) {
      container.innerHTML = animes
        .map((anime) => createAnimeCard(anime, "ongoing", "col-6 col-md-4 col-lg-3 mt-4"))
        .join("");
    } else {
      container.innerHTML = `<p class="text-secondary text-center">Tidak ada anime ongoing ditemukan.</p>`;
    }

    if (paginationContainer && pagination) {
      renderPagination(paginationContainer, pagination, "ongoing.html");
    }
  } catch (error) {
    showError(container, "Gagal memuat anime ongoing.");
  }
}
